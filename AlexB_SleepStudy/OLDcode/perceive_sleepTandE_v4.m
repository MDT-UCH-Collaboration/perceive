function [outMAT] = perceive_sleepTandE_v4(inPS)
...
%         actLFPdates = lfpDayPro2(ismember(lfpDayPro2,actDATES2));
        ...
        [outACT2LFPbins] = natAssess(actDAYcol)
...
%         lfpDateREF = actDAYcol;
        ...
        for di = 1:length(uniMDate)
            ...
            %             [hourTX12hr] = convertBT1224('LFP', hourCOLtx)
...
%             cell2table(act10minBin,'VariableNames',{'Date','Time','ActMean',...
%                 'ActSTD','WLMean','WLSTD','RLMean','RLSTD','GLMean','GLSTD',...
%                 'BLMean','BLSTD','WakeFrac','MaxState'});
...
function [outACT2LFPbins] = natAssess(inACT2LFPbins)
...
%     oneLocs = find(natDiff);

    if natLocs(1) == 1 % If first element is NAT
...
%     natRemL = zeros(size(natLocs));
% %     natRem = zeros(size(natLocs));
%     if oneLocs(1) == 1
%         natRemL(1) = 1;
%         natRemL(2:end) = oneLocs;
% %         natRem = natLocs(natRemL)
%     end

%     for ni = 1:length(natLocs)
